...
Pooled objects are cleared between GC cycles. See [sync.Pool](/docs/concurrency/sync-pool) for details.
...
`-alloc_objects` shows total allocations (not just live ones), which reveals hot allocation sites. See [pprof Basics](/docs/tooling/pprof-basics) for the full workflow.
