...
:::note
Maps are NOT safe for concurrent use. A concurrent read and write, or two concurrent writes, will be detected by the Go runtime and cause a fatal (unrecoverable) crash. See the [Map Concurrency](../concurrency/map-concurrency) article for the full explanation and solutions.
:::
