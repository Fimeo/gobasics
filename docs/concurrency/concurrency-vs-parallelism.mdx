...
:::warning
**"Goroutines are threads"** — Goroutines are multiplexed onto OS threads by the Go runtime scheduler. A goroutine is not an OS thread. See [Goroutine Scheduling](/concurrency/goroutine-scheduling) for the G/M/P model.
:::
